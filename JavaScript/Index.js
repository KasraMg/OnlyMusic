...
artists.slice(0, 6).map(info => (
  artistsDev.insertAdjacentHTML('beforeend',
    ...
    <a href='./Src/artist.html?artist=${info.englishName}&type=all&page=1' class="">
      <img src="${info.photo}" alt="cover" class="rounded-full mx-auto " style='height:140px;width:140px'>
    </a>

    <a href='./Src/artist.html?artist=${info.englishName}&type=all&page=1''  >
...
ourPlayList.forEach(item =>
  playListContent.insertAdjacentHTML(
    "beforeend", `
        ...
        <a href='./Src/mPlayList.html?type=ourPlayList&plId=${item.id}&id=${item.data[0].id}' class="w-full cursor-pointer">
        <div class="relative ">
...
window.addEventListener('load', () => {
  ...
  getInfoes('https://one-api.ir/radiojavan/?token=677668:64ae5b9d7c848&action=new_songs').then(data => {
    ...
    if (data.status == 200) {
      ...
      resultFilter.slice(0, 8).map(music => (
        newSong.insertAdjacentHTML(
          ...
          <div class="w-full">
    <div class="relative">
    <a  href='${music.type === 'mp3' ? `./Src/music.html` : `./Src/mVideo.html`}?artist=${music.artist}&id=${music.id}'><img  src=${music.photo} alt="cover" class="rounded-lg w-full object-cover" ${music.type === 'all' ? `style=height:16rem` : ''}></a> 
    ...
    <a href='${music.type === 'mp3' ? `./Src/music.html` : `./Src/mVideo.html`}?artist=${music.artist}&id=${music.id}' >
    ...
    <h4 class="font-semibold text-sm md:text-xs text-secondText">${music.artist_farsi}</h4>
      ...
      sliderToShow.map(music => (
        mySwiper1.insertAdjacentHTML(
          "beforeend", `
                          <a  href='./Src/music.html?artist=${music.artist}&id=${music.id}'  class="relative swiper-slide">
