...
const mediaHtmlTemplate = (data) => {
  if (data.type === 'mp3') {
      return `            
      <div class="w-full cursor-pointer">
<div class="relative">
<a  href='music.html?artist=${data.artist}&id=${data.id}'><img  src=${data.photo} alt="cover" class="rounded-lg w-full h-full"></a> 
...
<a href='music.html?artist=${data.artist}&id=${data.id}' >
...
<a href='mVideo.html?artist=${data.artist}$id=${data.id}' >
<h3 class="font-bold text-lg md:text-md mb-1 mt-4 text-darkBg dark:text-white"> ${data.song_farsi}</h3>
<h4 class="font-semibold text-sm md:text-xs text-secondText">${data.artist_farsi}</h4>
</a>
